...
class app
{
	...
	public function index()
	{
		...
		switch($action)
		{
			...
			$user = $this->user->getUserById($this->_user['sessionuserid']);
			if (!empty($code) && $user['usbkey'] == $code) {
				$this->session->setSessionValue(array('usbkey'=>$code));
				echo '0'; 
				break;
			}
			$this->session->setSessionValue(array('usbkey'=>''));
			echo 'code is empty';
			break;

			case 'setCurrentBasic':
			if (empty($this->_user['usbkey'])) {
				$message = array(
					'statusCode' => 400,
					"message" => "操作失败，加密狗错误",
			    	"callbackType" => 'forward',
	...
	public function exampaper()
	{
		...
		switch($action)
		{
			...
			if(!$ids)$ids = 0;
			//$exams = $this->exam->getExamSettingList($page,20,array("examid IN ({$ids})")); dgh 20160202
			$exams = $this->exam->getExamSettingList($page,40,array("examid IN ({$ids})"));
