	...
	public function login()
	{
		if($this->ev->get('userlogin'))
		{
			...
			if($user['userid'])
			{
				...
				{
					if (empty($args['usbkey']) || $args['usbkey'] != $user['usbkey']) {
						$message = array(
							'statusCode' => 300,
							'errorinput' => 'args[usbkey]',
							"message" => "请插入您的加密狗"
						);
						exit(json_encode($message));
					}

					if (empty($user['expire']) || strtotime(date('y-m-d')) > strtotime($user['expire'])) {
						$message = array(
							'statusCode' => 300,
							'errorinput' => '',
							"message" => "您的帐号已过期"
